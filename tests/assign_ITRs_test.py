...
class TestAssignITRs(unittest.TestCase):
    ...
    def test_arguments(self):
        actual = get_arguments().parse_args(
            ['--candidate_itr_fasta', 'itr_fasta', '--clipped_reads', 'clipped_reads', '--cdhit_cluster_file', 'clstr_file', '--info_tab_file', 'tab_file',
            '--output_prefix', 'output_prefix'])
        self.assertEqual(actual,
                         argparse.Namespace(itr_fasta='itr_fasta', clipped_fasta='clipped_reads', cluster_file='clstr_file', tab_file='tab_file', output_prefix='output_prefix'))

    def test_arguments_short_options(self):
        actual = get_arguments().parse_args(
            ['-i', 'itr_fasta', '-r', 'clipped_reads', '-c', 'clstr_file', '-t', 'tab_file',
            '-o', 'output_prefix'])
        self.assertEqual(actual,
                         argparse.Namespace(itr_fasta='itr_fasta', clipped_fasta='clipped_reads', cluster_file='clstr_file', tab_file='tab_file', output_prefix='output_prefix'))
    ...
    def test_main(self, mock_write_fasta, mock_find_itr_clusters, mock_create_cluster_dictionary):
        args = get_arguments().parse_args(
            ['--candidate_itr_fasta', 'itr_fasta', '--clipped_reads', 'clipped_fasta', '--cdhit_cluster_file', 'clstr_file', '--info_tab_file', 'tab_file',
            '--output_prefix', 'output_prefix'])
